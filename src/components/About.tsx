...
const About = () => {
  ...
  const submit = async () => {
    try {
      const result = await axios.post(`${serverUrl}/users`, {
        email,
      });
  ...
  return (
    <VStack alignItems="start">
      ...
      <Text fontSize="12px" fontWeight="bolder" lineHeight="30px">
        NOLIMIT, retail chain takes pride in its Sri Lankan roots but offers a
        world-class retail experience that drives customer loyalty while
        simultaneously setting new benchmarks in customer-centricity to take the
        art of retailing to a new level.
      </Text>
